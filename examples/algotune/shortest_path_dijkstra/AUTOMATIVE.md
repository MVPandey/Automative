...
Make `Solver.solve` compute all-pairs shortest path lengths in a weighted undirected graph as fast as
possible. The graph comes in CSR form (`data`, `indices`, `indptr`, `shape`); return
`{"distance_matrix": rows}` with `None` where there is no path.
...
reference's time divided by yours on 3 problems of size 160, after checking every answer with
...
The reference runs Dijkstra from every node and finds the next vertex with a linear scan over all
unvisited vertices, so each source costs quadratic time. Density is 0.2 and weights are integers 1 to
100. Distances are compared exactly (they are sums of integers).
